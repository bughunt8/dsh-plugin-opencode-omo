 ...
 * placeholder key env names and the models the importer expects.
 */
...
test('provider example describes the four omo.json providers with placeholder keys', async () => {
  const parsed = yaml.load(await readFile(EXAMPLE, 'utf8'))
  const providers = parsed.llm_pi_ai?.providers ?? parsed['llm-pi-ai']?.providers
  assert.ok(providers, 'expected a providers map')
  for (const name of ['apiyi', 'zai-coding-plan', 'moonshotai', 'minimax']) {
    const entry = providers[name]
    assert.ok(entry, `missing provider ${name}`)
    assert.match(String(entry.apiKeyEnv), /_API_KEY$/)
    assert.ok(String(entry.baseURL).includes('PLACEHOLDER'), `${name} baseURL must be a placeholder`)
