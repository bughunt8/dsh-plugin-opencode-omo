/**
 * omo.json import core.
 *
 * Pure, dependency-injected logic shared by the host routes, the startup
 * auto-import, and any CLI consumer.
 *
 * Two accepted file formats:
 *  1. Official oh-my-openagent omo.json: `{ "$schema": …, "[opencode]": {
...
import { isOmoRole } from './omo-roles.ts'
import type { OmoModelSelection, OmoRoleConfig } from './omo-roles.ts'
import { normalizeRoleConfig } from '../omo-role-registry.ts'
import type { StoredOmoRoleConfig } from './omo-roles.ts'
...
function toWireConfig(stored: StoredOmoRoleConfig): OmoRoleConfig {
  return {
