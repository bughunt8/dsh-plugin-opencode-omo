...
const DELEGATION_TABLE = [
  ['task / call_omo_agent', 'any independent, self-contained implementation/analysis unit; pick the role via category or subagent_type, resume with task_id=ses_...'],
  ['plan', 'consult the Plan Agent (Prometheus / ulw-plan) before any work of 2+ steps; it only writes the .omo/ plan, no implementation'],
  ...
  ['athena / athena-junior / council-member', 'multi-perspective committee review and adjudication'],
]
...
const KEY_TRIGGERS = [
  ...
  'Multi-perspective review → athena / athena-junior / council-member, or task(subagent_type="athena")',
]
