...
# Parallel dispatch (this harness has no team_* tools)

\`team_create\` / \`team_send_message\` / \`team_task_create\` are not mounted. Do **not** wait for them. Parallel \`task()\` / \`call_omo_agent\` replace the old team-mode protocol. Use \`workflow\` / \`ralph\` only when the steps are a staged pipeline rather than independent units.

## Phase 4: staffing labels still matter

When invoking the Plan agent in Phase 4.1, ask it to tag each atomic step:

- \`quick\`: mechanical edits — \`lsp_find_references\` then edit (this harness has no \`lsp_rename\`), extract variable, inline, simple move, signature change without call-site logic.
...
Recommend a parallel path when \`file_independent_steps >= 3\`; otherwise stay on the original 5.1 / 5.2 / 5.3 flow.

## Phase 5: parallel task() when the plan is independent enough

For each independent step, fire in one turn:

\`\`\`
task(category="quick", run_in_background=true, description="refactor step <N>",
     prompt=<per-step instructions + target files + line ranges + rollback + Intent Card + verify-spec>)
\`\`\`

Use \`category="quick"\` for mechanical steps and a generic / \`sisyphus-junior\` spawn for reasoning steps. After each \`<system-reminder>\`, collect with \`background_output(task_id="bg_...")\`, then verify OUTSIDE the worker:

\`\`\`
task(category="deep", run_in_background=true, description="verify step <N>",
     prompt=<files touched + verify-spec commands + return PASS or FAIL:...>)
\`\`\`

Do not commit until the verifier returns PASS. After three FAIL cycles on the same step, stop and consult the user. Lead orchestrates only — do not edit files in the lead session while workers are running.

## MUST NOT

- Do not call \`team_*\`.
- Do not run tests from Lead — the external verifier owns that lane.
- Do not put \`oracle\` / \`librarian\` inside a worker that is supposed to edit; call them via a separate \`task()\` when needed.
`
