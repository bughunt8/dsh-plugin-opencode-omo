---
name: start-work
description: "Execute a Prometheus work plan with Boulder state, evidence ledger updates, worktree discipline, and parallel subagents. Use after planning when the user says start work, execute plan, continue plan, resume plan, or asks to run a .omo/plans plan."
---
...
This session is the OpenCode + omo tool surface. Use only names in your current tool list:

- Review: `task(subagent_type="oracle", ...)` or `oracle(...)`
- Workers: `task(category="...", ...)` or `call_omo_agent(...)`
- Collect: `background_output(task_id="bg_...")` only after a completion notice
- Follow-up: `task(task_id="ses_...", prompt=...)`
- Team: `workflow` / `ralph`

Every child prompt must be self-contained (`TASK`, `DELIVERABLE`, `SCOPE`, `VERIFY`). Name `load_skills` inside the child prompt. Require `WORKING: <task> - <current phase>` before long passes and `BLOCKED: <reason>` only when progress stops. Do not call `multi_agent_v1.*`, `spawn_agent`, `job_output`, `job_kill`, `subagent`, or `subagent_fork`.

# start-work

Execute a Prometheus work plan until every top-level checkbox is complete. This skill pairs with the preset continuation hook: while `.omo/boulder.json` lists this session and the active plan still has unchecked column-0 boxes, the harness steers the next turn automatically. Do not stop to ask whether to continue.
...
Write `.omo/boulder.json` before implementation starts. Prefix session ids with `dsh:` so the continuation hook can identify its own session.
...
6. **DELEGATE EVERYTHING. YOU NEVER IMPLEMENT.** Dispatch ALL independent sub-tasks across those checkboxes in one parallel `task(...)` / `call_omo_agent(...)` burst; serialize only named dependencies. Verification and checkbox marking stay per-checkbox.
...
5. One Manual-QA channel, named with the exact tool and exact invocation (the literal `curl`, `send-keys`, playwright / agent-browser action, `page.click`, payload, selectors, and the binary observable that decides PASS/FAIL), not "verify it works". A LIGHT checkbox needs one real-surface proof of its deliverable, and auxiliary surfaces (CLI stdout, DB state diff, parsed config dump) are first-class when the surface is CLI- or data-shaped:
   ...
   - Browser use: drive the real page with the project's playwright / agent-browser / Chrome tooling (https://github.com/vercel-labs/agent-browser when no project browser is configured).
...
- The verifier must be independent from the executor: use `task(subagent_type="oracle", ...)` / `oracle(...)`, a scoped worker reviewer, or root only when root did not implement or materially rewrite that task.
...
- No unprefixed session ids in Boulder state. This harness prefixes them `dsh:<session_id>`.
