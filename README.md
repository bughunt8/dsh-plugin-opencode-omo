...
- **`/ulw` super-loop skill** — the generic ULTRAWORK "just say ulw and walk away" mode for any workflow and any database: it first assesses whether a Domain Map (covering ALL systems, workflows and databases in scope; concept reference: mattpocock's `domain-modeling` skill) would be best executed BEFORE the loops, then binds a per-codebase system map and a workflow state machine (items, stages, path matrix, stores, queues, final destination) and runs the full audit → root-cause → plan review → implementation → deploy pipeline with a repeated 30-minute soak until 5 evidence samples exist per acceptance lane, closed by an adversarial ORACLE gate that re-executes verification and reproduces artifact hashes, then a 5-lane review, commit, and final report.
