...
- **`/ulw` super-loop skill** — the omo "just say ulw and walk away" mode. One slash command runs ALL ultrawork phases with zero intervention: Loop A context + audit (evidence_ref objects, false-green post-mortem, persistent `.omo/state` ledger with carry-forward), Loop B root-cause + ONE decision-complete plan at `.omo/plans/<slug>.md` (Metis gap pass, then Momus + Oracle dual OKAY/REJECT review, max 5 rounds), Loop C Atlas execution (coordinator never implements; each boulder writes its own report), Loop D soak-and-prove (default 3 clean rounds; any failure resets the streak; per-proposition evidence samples), Loop E ORACLE acceptance gate (ORACLE re-executes the verification scripts and reproduces artifact hashes — narrative review alone does not pass).
